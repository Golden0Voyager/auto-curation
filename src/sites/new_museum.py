...
class NewMuseumParser(BaseSiteParser):
    ...
    def _extract_from_html(self, html: str, url: str) -> Optional[Dict[str, Any]]:
        ...
        date_match = re.search(
            r'([A-Za-z]+\s+\d{1,2},\s+\d{4})\s*[-–—]\s*(Ongoing|[A-Za-z]+\s+\d{1,2},\s+\d{4})',
            text
        )
        ...
        preface = None
        for line in lines:
            if len(line) > 80 and not any(k in line for k in ['Skip To', 'Home Page', 'Visitor Guidelines', 'Accessibility', 'Tours', 'Restaurant', 'Exhibitions', 'Events', 'Learn', 'About', 'Support', 'Shop', 'SEARCH', 'TICKETS', 'New Museum Home Page']):
                preface = line
                break
        ...
        if curator_match:
            curators = [c.strip() for c in curator_match.group(1).split(',') if c.strip() and 'Curator' not in c]
