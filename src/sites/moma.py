...
class MoMAParser:
    ...
    def get_exhibition_urls(self, client: httpx.Client, since_year: Optional[int] = None) -> List[str]:
        ...
        logger.info(f"[MoMA] Using GitHub open dataset (not web scraping). Use get_csv_exhibitions() directly.")
        return []

    def get_csv_exhibitions(self, since_year: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if os.path.exists(MOMA_LOCAL_CSV_PATH):
                ...
                # 本地文件不存在，从 GitHub 下载
                logger.info(f"[MoMA] Local cache not found. Downloading from GitHub...")
                client = httpx.Client(timeout=60.0, follow_redirects=True)
                response = client.get(MOMA_EXHIBITIONS_CSV_URL, headers={
                    "User-Agent": "Mozilla/5.0 (compatible; auto_curation bot)"
                })
            ...
            for row in reader:
                ...
                if ex_id not in groups:
                    ...
                    if since_year and begin_date:
                        try:
                            begin_year = int(begin_date.split("/")[-1]) if "/" in begin_date else int(begin_date[:4])
                ...
                if "Curator" in role or "Director" in role or "Organiz" in role:
                    ...
                    groups[ex_id]["artworks"].append({
                        ...
                        "work_title": title,
                        "work_year": groups[ex_id]["start_date"][:4] if groups[ex_id]["start_date"] else None,
                        "medium": None,
                        "dimensions": None,
            ...
            logger.info(f"[MoMA] Parsed {len(exhibitions)} exhibitions from dataset (since_year={since_year}).")
            return exhibitions
