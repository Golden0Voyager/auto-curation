...
def get_timeline():
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, source, start_date FROM exhibitions WHERE start_date != '' AND start_date IS NOT NULL"
        )
        ...
        return {"years": sorted_years, "museums": sources_list, "series": series_data}
    finally:
        conn.close()
...
def get_network(limit_artists: int = 100, min_cooccurrence: int = 2):
    ...
    try:
        ...
        for row in links_rows:
            ...
            links.append({"source": row["source"], "target": row["target"], "value": row["weight"]})
            ...
            nodes.append(
                {
                    ...
                    "symbolSize": max(
                        6, min(22, 6 + (top_artists.get(name, 1) ** 0.5) * 1.0)
                    ),  # Flat square root model to prevent huge bubbles
                }
            )
...
def get_exhibitions(
    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ex_data["curators"] = (
                    json.loads(ex_data["curators"]) if ex_data.get("curators") else []
                )
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM artworks WHERE exhibition_id = ?", (ex_data["id"],)
            )
            ex_data["artwork_count"] = cursor.fetchone()[0]
            exhibitions.append(ex_data)

        return {"total": total_count, "limit": limit, "offset": offset, "data": exhibitions}
    finally:
        conn.close()
...
def get_index():
    ...
    return HTMLResponse(
        content="<h1>CurationInsight Dashboard</h1><p>Index file templates/index.html not found.</p>"
    )
