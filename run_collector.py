...
def list_registered_sites():
    ...
    for key, parser in sorted(SITES.items()):
        ...
        has_history = bool(
            ...
            or strategy
            in (
                ParserStrategy.CSV_LOCAL,
                ParserStrategy.CSV_REMOTE,
                ParserStrategy.REST_API,
                ParserStrategy.SPARQL,
            )
        )
        ...
        print(
            f" - {key:<18}: {parser.source:<26} ({parser.city}) | {strategy_label:<12} | {hist}{type_badge}"
        )
    print("-" * 70)
...
def main():
    ...
    arg_parser.add_argument(
        "--limit", type=int, default=None, help="每个机构最多处理的展览页面数（不设则全量抓取）"
    )
    arg_parser.add_argument(
        "--force", action="store_true", help="重新抓取已存入 DB 的展览（跳过去重检查）"
    )
    arg_parser.add_argument(
        "--dry-run", action="store_true", help="模拟运行：只抓取和解析，不写入数据库"
    )
    arg_parser.add_argument("--verbose", action="store_true", help="输出详细 DEBUG 日志")
    arg_parser.add_argument(
        "--concurrent", action="store_true", help="启用异步并发采集（仅对 HTML 策略生效）"
    )
    arg_parser.add_argument(
        "--db",
        type=str,
        default="exhibitions.db",
        help="SQLite 数据库文件路径（默认: exhibitions.db）",
    )
